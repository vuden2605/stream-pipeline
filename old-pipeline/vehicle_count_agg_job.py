...
# Redis — publish speed trực tiếp cho valhalla-hcm-traffic/traffic_updater.py
# Contract khớp scripts/publish_traffic.py bên đó: HASH field=edge_id,
# value="{speed}:{ts_giây}", rồi PUBLISH "update" trên REDIS_CHANNEL.
REDIS_HOST             = os.getenv("REDIS_HOST",            "redis")
REDIS_PORT             = int(os.getenv("REDIS_PORT",        "6379"))
REDIS_DB               = int(os.getenv("REDIS_DB",          "0"))
REDIS_KEY              = os.getenv("REDIS_KEY",              "traffic:speeds")
REDIS_CHANNEL          = os.getenv("REDIS_CHANNEL",          "traffic:events")

# Gom batch trước khi publish Redis thay vì ghi rời rạc từng edge:
#   - publish sớm ngay khi đã gom đủ TOTAL_EDGES edge khác nhau, HOẶC
#   - publish khi quá BATCH_WINDOW_MS kể từ bản ghi đầu tiên của batch,
...
BATCH_WINDOW_MS        = int(os.getenv("BATCH_WINDOW_MS",    str(800_000)))
...
# ── Redis batch publisher (traffic_realtime → valhalla-hcm-traffic) ─────────
#
#  Gom speed của nhiều edge vào 1 batch trước khi ghi Redis, thay vì ghi rời
#  rạc từng edge một. key_by(hằng số) dồn toàn bộ stream về đúng 1 instance
#  logic — bắt buộc để "nhìn thấy" mọi edge cùng lúc, nên set parallelism=1.
#  Chấp nhận được vì input đã bị AdaptiveEwmaFunction throttle còn tối đa
#  1 record/edge/EMIT_INTERVAL_MS (mặc định 30s), không phải luồng dày.
#
#  Điều kiện publish (OR — cái nào tới trước):
#    - batch đã gom đủ TOTAL_EDGES edge khác nhau kể từ lần publish trước
#    - đã quá BATCH_WINDOW_MS kể từ bản ghi đầu tiên của batch hiện tại
#
#  Khi publish: DELETE sạch REDIS_KEY rồi HSET lại toàn bộ batch (dọn dẹp +
#  ghi mới trong 1 pipeline), sau đó PUBLISH đúng 1 lần trên REDIS_CHANNEL —
#  không còn cảnh N edge cùng tick gây N lần PUBLISH dư thừa như bản trước.
#
class RedisBatchPublisher(KeyedProcessFunction):
    ...
    def open(self, runtime_context):
        ...
        batch_desc = MapStateDescriptor("redis_batch", Types.STRING(), Types.STRING())
        self.batch = runtime_context.get_map_state(batch_desc)

        timer_desc = ValueStateDescriptor("redis_batch_timer_ts", Types.LONG())
        self.timer_ts = runtime_context.get_state(timer_desc)

    def process_element(self, value: str, ctx: "KeyedProcessFunction.Context"):
        msg = json.loads(value)
        gps = msg.get("gps")
        if not gps or gps.get("avg_speed_kmh") is None:
            return  # valhalla chỉ cần speed — edge chỉ có camera thì bỏ qua

        edge_id = msg["edge_id"]
        ts      = msg["timestamp"] // 1000  # ms → giây, khớp publish_traffic.py
        speed   = gps["avg_speed_kmh"]
        self.batch.put(edge_id, f"{speed}:{ts}")

        if self.timer_ts.value() is None:
            fire_at = ctx.timer_service().current_processing_time() + BATCH_WINDOW_MS
            ctx.timer_service().register_processing_time_timer(fire_at)
            self.timer_ts.update(fire_at)

        batch_size = sum(1 for _ in self.batch.keys())
        if batch_size >= TOTAL_EDGES:
            # Gom đủ trước hạn → publish ngay, huỷ timer đang chờ để khỏi
            # flush rỗng thêm 1 lần nữa lúc timer đến hạn.
            ctx.timer_service().delete_processing_time_timer(self.timer_ts.value())
    ...
    def _flush(self):
        entries = {k: v for k, v in self.batch.items()}
        self.batch.clear()
        self.timer_ts.clear()
        ...
        pipe.execute()
        self.client.publish(REDIS_CHANNEL, "update")

        yield json.dumps({"published_edges": len(entries), "edge_ids": list(entries.keys())})
...
def main():
    ...
    # ── Batch + publish Redis (valhalla-hcm-traffic) ───────────────────────
    # key_by hằng số → dồn về 1 instance duy nhất để gom được nhiều edge
    # trước khi publish (xem docstring RedisBatchPublisher).
    redis_batches = (
        realtime
        .key_by(lambda x: "ALL")
        .process(RedisBatchPublisher(), output_type=Types.STRING())
        .set_parallelism(1)
    )
    redis_batches.print()
